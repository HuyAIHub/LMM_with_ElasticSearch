...
class ExtractProduceAction(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        # You can now use this name in your bot logic
        products = ["bảo hành", "chính sách bảo hành", "bao hanh", "chinh sach bao hanh", "chính sách"]       
        if any(p in produce_name_policy.lower() for p in products):
...
class ExtractProduceInventory(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        if any(p in produce_name_inventory.lower() for p in products):
            dispatcher.utter_message(text=f"""
                Anh/chị vui lòng nhập mã sản phẩm và mã tỉnh theo mẫu sau:
                """)
        else:
            dispatcher.utter_message(text="Anh/chị xin thông cảm! Em không hiểu yêu cầu đưa ra.")
...
class ExtractSearchProductSimilar(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        if any(p == search_product_similar.lower() for p in products):
            ...
            dispatcher.utter_message(text="Anh/chị xin thông cảm! Hiện tại tôi không tìm thấy sản phẩm tương tự.")
